...
if (isset($_GET['action']) && $_GET['action'] == 'add' && isset($_GET['id'])) {
    ...
    header("Location: pembeli_kue.php?success=true");
    exit;
}

// 2. Menangkap sinyal sukses untuk memunculkan notifikasi
if (isset($_GET['success']) && $_GET['success'] == 'true') {
    $pesan_sukses = "Kue berhasil ditambahkan ke Pesanan!";
}

$query_kue = mysqli_query($conn, "SELECT * FROM menu WHERE id_kategori = 1");
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <title>Menu Kue - Kue Balok Lumer GenZ</title>
        ...
        <aside class="w-[220px] bg-white px-5 py-6 shadow-sm border-r border-gray-200 shrink-0 flex flex-col">
            ...
            <ul class="space-y-3">
                <a href="pembeli_kue.php"
                    class="bg-[#5e3a21] text-white py-2 px-4 rounded-lg font-medium text-sm text-center block shadow-sm">Kue</a>
                ...
                <?php while ($kue = mysqli_fetch_assoc($query_kue)): ?>
                    <div
                        ...
                        <div class="bg-gray-100 h-40 w-full overflow-hidden flex items-center justify-center">
                            <?php if (!empty($kue['gambar']) && $kue['gambar'] != 'default.jpg'): ?>
                                <img src="uploads/<?= $kue['gambar'] ?>" alt="<?= $kue['nama_menu'] ?>"
                        ...
                        <div class="p-4 flex flex-col flex-1">
                            ...
                            <div class="mt-auto flex justify-between items-center">
                                <span class="font-semibold text-[#5e3a21] text-sm">Rp
                                    <?= number_format($kue['harga'], 0, ',', '.') ?>
                                </span>
                                <?php if ($kue['status'] != 'Habis'): ?>
                                    <a href="pembeli_kue.php?action=add&id=<?= $kue['id'] ?>"
